...
OfflinePath = '/home/kyle_thomas/Documents/For_Others/ME/SMU/RTLS_CaseStudy/Data/offline_data.txt'
...
onlinePath = '/home/kyle_thomas/Documents/For_Others/ME/SMU/RTLS_CaseStudy/Data/online_data.txt'
...
mapply(
  surfaceSS,
  mac = submac[rep(c(1, 2, 3, 4, 5, 6, 7), each = 4)],
  angle = rep(c(0, 135), 2),
...
AP_orig = matrix(
  c(7.5, 6.3, 7.5, 6.3, 2.5,-.8, 12.8,-2.8,
    1, 14, 33.5, 9.3,  33.5, 2.8),
  ncol = 2,
  byrow = TRUE,
  #             dimnames = list(submac[ -2 ], c("x", "y") ))
...
xyplot(
  signal ~ dist | factor(mac) + factor(angle),
  data = offlineSummary,
  pch = 19,
  cex = 0.3,
  xlab = "distance"
...
selectTrain = function(angleNewObs,
                       signals = NULL,
                       m = 1) {
...
estFold = predXY(
  newSignals = onlineFold[, 6:12],
  #changed to 6:12 to get all MACS
...
text(
  x = kMin - 2,
  y = rmseMin + 40,
  label = as.character(round(rmseMin)),
  col = grey(0.4)
)
# par(oldPar)
# dev.off()
